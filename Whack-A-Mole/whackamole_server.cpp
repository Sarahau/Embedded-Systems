...
unsigned long timerDelay = 600; // 5000; 5 minutes (300,000ms) or 5 seconds (5,000ms)


// buttons
Button whackerButton(20, sHeight- 75, 120, 55, "Whacker");
Button popperButton(sWidth- 140, sHeight- 75, 120, 55, "Popper");  
        ...
        } else if (thisDeviceGameState == INGAME)
        {
            ...
            case WHACKER:
                ...
                else
                {
                    ...
                    int othersTurnsRemaining = result / 10;
...
bool allMolesDown() {
    for (int i = 0; i < DICE_DIGITS; i++) {
        if (moles[i]) {
            return false; // one mole that was up was found
        }
    }
    return true;
}
...
bool gameOver()
{
    ...
    {
        return othersTurnsRemaining == 1; // the whacker sends 1 before updating their turnsRemaining to 0
    }
}
...
void drawIntroScreen(){        
        ...
        M5.Lcd.drawRoundRect(popperButton.x, popperButton.y, popperButton.w, popperButton.h, 10,TFT_DARKGREEN);
        M5.Lcd.println("Popper");

        M5.Lcd.setCursor(40, sHeight-55);
        whackerButton = Button(20, sHeight- 75, 120, 55, "Whacker");
        M5.Lcd.drawRoundRect(whackerButton.x, whackerButton.y, whackerButton.w, whackerButton.h, 10, TFT_MAROON);
...
void drawEndGameScreen() {
    ...
    if((thisDevicePlayer == WHACKER && allMolesDown()) || 
    (thisDevicePlayer == POPPER && !allMolesDown())){
        //You won
    ...
    // if ((millis() - lastTime) > timerDelay * 2) {
...
void buttonAction(Event& e) {
    ...
    // only do something with the buttons if we're in the set up state
    if (thisDeviceGameState == SETUP) {
        // Whacker Button
        if(b.instanceIndex() == 4) {
            Serial.println("\t\twhacker");
            thisDevicePlayer = WHACKER;
            isMyTurn = false; // whacker goes second
