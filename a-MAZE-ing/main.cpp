    ...
    if (screenState == MAZE)
    {
        if (((millis() - lastTime) > timerDelayMs))
        {
            ...
            if (mazeFloorPlan[currentY][currentX].floor == ICE)
            {
                ...
                if (iceMeltTemp == 0)
                {
                    // set the melting temp to 2 C higher than current
...
void drawMaze()
{
    ...
    for (int row = 0; row < height; row++)
    {
        for (int col = 0; col < width; col++)
        {
            ...
            {
                drawFlowerBud(convertCoor(col), convertCoor(row), TFT_MAGENTA);
            }
...
void drawEndScreen()
{
    ...
    M5.Lcd.setCursor(sWidth / 5, sHeight / 3 - 10);
    M5.Lcd.setTextColor(TFT_WHITE);
    M5.Lcd.setTextSize(3);
    M5.Lcd.println("You did it!");

    M5.Lcd.setCursor(sWidth / 5, sHeight / 2 - 10);
    M5.Lcd.setTextSize(2);
    ...
    M5.Lcd.setCursor(sWidth / 5 + 15, (sHeight / 2) + 20);
