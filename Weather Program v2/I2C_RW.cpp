...
////////////////////////////////////////////////////////////////////
int I2C_RW::i2cVcnl4040Address = 0;
int I2C_RW::i2cSht40Address = 0;
...
//////////////////////////////////////////////////////////////////////
void I2C_RW::initI2C(int i2cVcnl4040Addr, int i2cSht40Addr, int i2cFreq, int pinSda, int pinScl) {
    // Save parameters in case needed later
    i2cVcnl4040Address = i2cVcnl4040Addr;
    i2cSht40Address = i2cSht40Addr;
...
uint16_t I2C_RW::readReg8Addr16Data(byte regAddr, int numBytesToRead, String action, bool verbose) {
    ...
    for (int i = 0; i < maxRetries; i++) {
        ...
        Wire.beginTransmission(i2cVcnl4040Address);
        ...
        Wire.requestFrom(i2cVcnl4040Address, numBytesToRead);
...
// Read register at the 8-bit address via I2C; returns the 16-bit value
//////////////////////////////////////////////////////////////////////
void I2C_RW::getSht40Data(byte regAddr, float* temperature, float* humidity) {
    String action = " getting Sht40 sensor data";
    int numBytesToRead = 6;
    // Attempts a number of retries in case the data is not initially ready
    int maxRetries = 50;
    for (int i = 0; i < maxRetries; i++) {
        // Enable I2C connection
        Wire.beginTransmission(i2cSht40Address);
        ...
        // Read data from above address
        Wire.requestFrom(i2cSht40Address, numBytesToRead);
        ...
        if (Wire.available() == numBytesToRead) {
            ...
            float _temp = -45 + 175 * t_ticks / 65535;
            float _hum = -6 + 125 * rh_ticks / 65535;

            // convert temperature from C to K
            *temperature = _temp + 273.15;
            *humidity = _hum;
        }
    }
}
