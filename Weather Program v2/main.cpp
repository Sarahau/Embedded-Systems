...
// VCNL4040
const byte vcnl4040Address = 0x60; // vcnl 4040
const byte vcnlRegProx = 0x08;
const byte vcnlRegAls = 0x09;
const byte vcnlRegPsConfig = 0x03;
const byte vcnlRegAlsConfig = 0x00;
const int CLOSE_PROXIMITY = 150;

// SHT40
const byte poop40Address = 0x44;      // sht40
const byte poopTempHumCommand = 0xFD; // low precision

// Sensor Data variables
float sensorTemp;
float sensorHum;

// temporary variables for data to use when requesting data, to be checked against the stored values
// API data
double newTempNow;
double newTempMin;
double newTempMax;
...
void setup()
{
    ...
    // sensor reading
    I2C_RW::initI2C(vcnl4040Address, poop40Address, I2C_FREQ, I2C_SDA_PIN, I2C_SCL_PIN);

    I2C_RW::writeReg8Addr16DataWithProof(vcnlRegPsConfig, 2, 0x0800, " proximity sensor", true);
    I2C_RW::writeReg8Addr16DataWithProof(vcnlRegAlsConfig, 2, 0x0000, " ambient light sensor", true);
}
...
void loop()
{
    ...
    if ((millis() - lastTime) > timerDelay)
    {
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            fetchWeatherDetails();
            if (screen == S_WEATHER && redrawAPI)
            {
                // the API values have changed so update them!
                drawWeatherDisplay();
            }
        }
        else
        ...
        // Sensor things (aka S_SENSOR)
        readSht40Data(); // check local sensor readings

        if (screen == S_SENSOR && redrawSensor)
        {
            // the sensor values have changed so update them!
            drawSensorDisplay();
        }
    ...
    // checking if the VCNL4040 sensor is too close to an object (proximity)
    u16_t proxData = I2C_RW::readReg8Addr16Data(vcnlRegProx, 2, " to get proximity", false);
    if (proxData > CLOSE_PROXIMITY) {
        // too close, turn off screen
        // Serial.println("OFF");
        M5.Lcd.sleep();
    } else {
        // Serial.println("\t\tON");
    ...
    // checking the VCNL4040 sensor ambient white light
    u16_t ambientData = I2C_RW::readReg8Addr16Data(vcnlRegAls, 2, " to get ambient light", false);
    
    // ambientData ranges from 0(DARK) to 65535 (BLINDING LIGHT)
    long lowAmbientLight = 0.0;
    long highAmbientLight = 65535.0;

    // M5 LCD screen voltage range(2500-3300) (800 numbers)
    long lowScreenVolt = 2500.0;
    long highScreenVolt = 3300.0;
    long screenVoltage = ((highScreenVolt - lowScreenVolt) * 1.0 / (highAmbientLight - lowAmbientLight) * 1.0) * 1.0 * ambientData + lowScreenVolt;
    Serial.printf("\tSetLcdVoltage: %d \t\t ambient data: %d \n", screenVoltage, ambientData);
    ...
    {
        if (screen == S_WEATHER || screen == S_SENSOR)
        {
            screen = S_ZIP;
            drawZipCodeDisplay();
        }
...
void fetchWeatherDetails()
{
    ...
    newTempNow = objMain["temp"];
    newTempMin = objMain["temp_min"];
    newTempMax = objMain["temp_max"];

    // Check if API temperature values have changed
    if (newTempNow == tempNow && newTempMin == tempMin && newTempMax == tempMax)
    {
        // Serial.println("API DATA IS THE SAME, NO NEED TO REDRAW");
        redrawAPI = false;
    }
    else
    {
...
void readSht40Data()
{
    I2C_RW::getSht40Data(poopTempHumCommand, &newSensorTemp, &newSensorHum);

    // Check if sensor variables are different
    if (newSensorTemp == sensorTemp && newSensorHum == sensorHum)
    {
        // Serial.println("SENSOR DATA IS THE SAME, NO NEED TO REDRAW");
        redrawSensor = false;
    }
    else
    {
        sensorTemp = newSensorTemp;
        sensorHum = newSensorHum;
        redrawSensor = true;
    }
}
...
void drawSensorDisplay()
{
    ...
    {
        units = 'F';
        sensorT = toFahrenheit(sensorTemp);
    }
    else if (unit == U_CELSIUS)
    {
        units = 'C';
        sensorT = toCelcius(sensorTemp);
    }
    ...
    M5.Lcd.setTextSize(2);
    M5.Lcd.setCursor(pad, pad);
    M5.Lcd.print("Live Local Readings:\n");
    ...
    M5.Lcd.setTextSize(4);
    M5.Lcd.printf("%0.f%%", sensorHum);

    M5.Lcd.setCursor(pad, sHeight - 20);
    M5.Lcd.setTextColor(TFT_DARKGREY);
    M5.Lcd.setTextSize(2);
    M5.Lcd.printf("Last updated: %02d:%02d:%02d %s", hours, minutes, seconds, amPm);
}
